...
＜エネルギーベース最適化の手順＞
takahashi_attenuation_EMFF.mでリアプノフ制御を用いて磁気モーメント入力のノミナル軌道を作成する。d_avoid強制変更中
scp_mpc_EMFF_nonl_stabilization_min_energy_fmincon2.mでノミナル軌道を変数の初期値としてfminconで最適化する。cvxよりもfminconの方が速い。d_avoid強制変更中なので、制約条件を守って最適化ができていない。
scp_mpc_EMFF_nonl_optimal_input.mである状態が与えられたときに、最適な入力を計算する。
scp_mpc_EMFF_nonl_optimal_input_func.mはscp_mpc_EMFF_nonl_optimal_input.mの関数化のテスト

scp_mpc_EMFF_nonl_stabilization_soft.mでscp_mpc_EMFF_nonl_stabilization_min_energy_fmincon2の進入禁止制約をsoftに変換。スラック変数を導入。xの後半の値がマイナスだったらその分進入禁止範囲が小さくなっている。
...
L119~L122を変えるとエナジーベースと最適化ベースで変えられる。
L1031~L1033で何回scpするかを変えられる。
L864でペア決めをやってる
