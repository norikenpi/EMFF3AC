function createAnimation_x(histories, param, satellites)
    fps = 31;
    pause = 300;
    filename_option = "x";
    ...
    for i = 1:round(pause/(fps*param.dt)):int32(param.finished_time/param.dt)
        ...
        text(xPosition(1), yPosition(1)*2, zPosition(1)/3, stringparam.date), 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom');
        
        %時間を表示
        text(xPosition(1), yPosition(1)*2, zPosition(1)/2, sprintf("time %s", string(i*param.dt)), 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom');
        drawnow;
