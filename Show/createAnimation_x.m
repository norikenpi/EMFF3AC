function createAnimation_x(histories, param, satellites)
    ...
    for i = 1:round(pause/(fps*param.dt)):size(histories.position_histories, 1)
        ...
        text(xPosition(1), yPosition(1)*2, zPosition(1)/3, string(param.date), 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom');
        
        %時間を表示
        text(xPosition(1), yPosition(1)*2, zPosition(1)/2, string(i*param.dt), 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom');
        drawnow;
