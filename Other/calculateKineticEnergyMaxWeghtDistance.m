function weight_distance = calculateKineticEnergyMaxWeghtDistance(param)
    param.t = 0.1;
    [satellites, ~ , ~] = simulateSatellite(param);
    KE_max = 0; %Kinetic Energy
    i_max = 0;
    
    for i = 1:param.N
        KE = norm(satellites{i}.velocity)/2; 
        %KE = abs((norm(satellites{i}.velocity)^2)/2 + (param.n*satellites{i}.position(2)^2)/2 - (3*param.n*satellites{i}.position(3)^2)/2);
        if KE > KE_max
            KE_max = KE;
            i_max = i;
        end
